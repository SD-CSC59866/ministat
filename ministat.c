...
ReadSet(const char *n)
{
	...
	if (flag_v) {
	  ...
	  accum[1] = stop.tv_sec - start.tv_sec;
	}
...
usage(char const *whine)
{
	...
	fprintf(stderr, "%s\n", whine);
	fprintf(stderr,
			"Usage: ministat [-C column] [-c confidence] [-d delimiter(s)] [-ns] [-v timing ][-w width] [file [file ...]]\n");
...
int main(int argc, char **argv)
{
	...
	if (argc == 0) {
		...
		i.file = "-";
		i.flag_v = flag_v;
		pthread_create(&thread[0], NULL, &ReadSet, (void*)&i);
